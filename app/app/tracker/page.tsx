...
export default function TrackerPage() {
  ...
  const kpis = [
    { label: "Total", value: counts.total, color: "white" },
    { label: "Drafting", value: counts.drafting, color: "#71717a" },
    ...
    { label: "Rejected", value: counts.rejected, color: "#71717a" },
  ];
  ...
  return (
    <PageShell>
      ...
      <div className="grid gap-3 xl:grid-cols-7">
        {kpis.map((item) => (
          <Panel className="border-zinc-800 bg-zinc-900 px-4 py-4" key={item.label}>
        ...
        {statusTabs.map((item) => (
          <button
            className={`rounded-full border px-3 py-1.5 text-xs ${
              activeTab === item ? "border-white bg-white text-black" : "border-zinc-800 bg-zinc-950 text-zinc-500"
            }`}
            key={item}
      ...
      <Panel className="overflow-hidden p-0">
        <PanelHeader className="border-b border-zinc-800 px-4 py-4">
          <PanelTitle>Tracked programs</PanelTitle>
        </PanelHeader>
        <div className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr] border-b border-zinc-800 px-4 py-3 text-[11px] uppercase tracking-[0.14em] text-zinc-600">
          <div>Program</div>
          <div>Status</div>
          <div>Deadline</div>
        ...
        {filteredPrograms.length === 0 ? (
          ...
          filteredPrograms.map((program) => (
            <div
              className="grid grid-cols-[1.4fr_1fr_1fr_1fr_1fr] items-center border-l-2 px-4 py-4 text-[14px] text-zinc-400 transition-colors hover:bg-zinc-900"
              key={program.slug}
              style={{
                borderLeftColor:
      ...
      {modalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
          <div className="w-full max-w-[680px] rounded-[20px] border border-zinc-800 bg-black p-6">
            ...
            <div className="mt-6 space-y-3">
              {availablePrograms.map((program) => (
                <div
                  className="flex items-center justify-between gap-4 rounded-[14px] border border-zinc-800 bg-zinc-950 px-4 py-4"
                  key={program.slug}
                >
                  <div className="flex items-center gap-3">
                    <ProgramLogo domain={program.domain} size={36} slug={program.slug} />
                    <div>
                    <div className="text-[15px] font-medium text-white">{program.name}</div>
                    <div className="mt-1 text-[13px] text-zinc-500">{program.type}</div>
                    </div>
                  </div>
                  <Button
                    onClick={() => {
