...
The exact folder structure will evolve within the current `lib/` directory. The domain boundaries must not be replaced by a speculative rewrite. Currently only `lib/supabase.ts`, `lib/utils.ts`, `lib/demo-data.ts`, and `lib/startup-programs.ts` exist.
