...
async function runStagingE2E() {
  console.log("Starting Staging E2E Browser Test...");
  const version = await fetch("http://127.0.0.1:9222/json/version").then((r) => r.json());
  
  // Create a new target page
  const target = await fetch("http://127.0.0.1:9222/json/new?https://staging.tryfundme.in", { method: "PUT" }).then((r) => r.json());
  console.log("Created page target:", target.id, target.webSocketDebuggerUrl);
  ...
  console.log("1. Landing Page Title:", landingTitle);

  // Navigate to /assessment
  await cdp.send("Page.navigate", { url: "https://staging.tryfundme.in/assessment" });
  await new Promise((r) => setTimeout(r, 3000));
  console.log("2. Navigated to Assessment intake page");
  ...
  const submitRes = await cdp.eval(`(() => {
  ...
  console.log("4. Submitted intake form:", submitRes);

  // Poll for result completion
  console.log("5. Waiting for assessment analysis to complete...");
  let reportData = null;
  for (let i = 0; i < 40; i++) {
  ...
  if (!reportData) {
    throw new Error("Timed out waiting for assessment result");
  }
  console.log("6. Analysis Completed Successfully!");
  ...
  const saveClick = await cdp.eval(`(() => {
  ...
  console.log("7. Clicked 'Save my assessment':", saveClick);
  await new Promise((r) => setTimeout(r, 1000));

  // Click 'Continue with Preview profile'
  const continueClick = await cdp.eval(`(() => {
  ...
  console.log("8. Clicked 'Continue with Preview profile':", continueClick);
  ...
  console.log("9. Loaded Saved Dashboard (/app/preview):", dashboardState);

  // Perform Hard Refresh
  console.log("10. Testing Hard Refresh persistence...");
  ...
  console.log("11. Restored Workspace after Hard Refresh:", restoredState);

  if (restoredState.score !== reportData.score) {
    throw new Error("Score changed after refresh!");
  }
  console.log("   - Score accurately preserved:", restoredState.score);

  // Test Public Sharing Generation
  console.log("12. Testing Public Sharing Generation on Staging...");
  ...
  console.log("13. Share API response:", shareRes);

  await cdp.close();
  console.log("=== STAGING E2E AUDIT COMPLETE: ALL STEPS PASSED ===");
}
